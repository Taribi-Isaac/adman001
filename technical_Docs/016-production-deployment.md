...
| Steps | Checkout → PHP **8.4** + Node 22 → `composer install` → `.env` + key → `npm ci` → `npm run build` → `npm run types:check` → `composer test` (Pint + PHPStan + Pest) |
| Notes | No MySQL service in CI — Pest uses `sqlite :memory:` from `phpunit.xml`. Production still uses MySQL on the Droplet. `npm run check` (oxfmt) is deferred until a dedicated formatting pass; it currently fails on the existing tree. |
