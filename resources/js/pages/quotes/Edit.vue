...
const form = useForm({
    contact_id: props.quote.contact?.id ?? '',
    expiry_date: props.quote.expiry_date ?? '',
    discount_type: props.quote.discount_type,
    discount_value: props.quote.discount_value,
    tax_enabled: props.quote.tax_enabled,
...
const formatMoney = (amount: number) =>
    `${props.quote.currency_code} ${amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
...
<template>
    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
        <Heading :title="`Edit ${quote.number}`" description="Only draft quotes can be edited." />
